				  ...
				  $f = fopen($csv['tmp_name'], 'r') or die("ERROR OPENING DATA");
					...
					{ 
						...
						if($key!=0)
						{
							...
							$second_tamp_grn_records->processed      = $data[4]; 
							$second_tamp_grn_records->is_addition_item_data_required = $data[5];
							$second_tamp_grn_records->item_name      = $data[6]; 
							$second_tamp_grn_records->hsn_code       = $data[7];
							$second_tamp_grn_records->unit           = $data[8];
							$second_tamp_grn_records->gst_rate_fixed_or_fluid = $data[9];
							$second_tamp_grn_records->first_gst_rate = $data[10];
							$second_tamp_grn_records->amount_in_ref_of_gst_rate = $data[11];
							$second_tamp_grn_records->second_gst_rate = $data[12];
